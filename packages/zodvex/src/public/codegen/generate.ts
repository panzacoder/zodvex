...
/**
 * Memoizing helper emitted above the registry. Registry entries are getters
 * that call this so each entry's schemas are constructed on first access and
 * reused afterwards (stable identity per module instance).
 */
const LAZY_HELPER = `const __memo = new Map()
...
export function generateApiFile(
  ...
  // Build registry entries as memoizing getters. Every function file
  // imports the app's `initZodvex` wrapper, which imports this registry, and
  // the Convex isolate evaluates module top-level code per execution — so an
  // eager object literal would construct every function's Zod schemas on
  // every call. The registry is only ever read by key, so each entry builds
  // on first access and is memoized. Getters are own enumerable properties:
  // `Object.keys` / `Object.entries` still see every path (and build them).
  const entries = functions
    .map(fn => {
      const args = resolveSchema(fn.zodArgs)
      const returns = resolveSchema(fn.zodReturns)
      const key = `'${fn.functionPath}'`
      return `  get ${key}() {\n    return __lazy(${key}, () => ({\n      args: ${args},\n      returns: ${returns},\n    }))\n  }`
    })
    .join(',\n')
  ...
  const lazySection = entries.length > 0 ? `${LAZY_HELPER}\n` : ''
  const js = `${HEADER}\n${importSection}${codecVarSection}${lazySection}export const zodvexRegistry = {\n${registryEntries}}\n`
