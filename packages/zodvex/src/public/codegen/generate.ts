...
import { schemaSubtreeHasCodec, walkModelCodecPaths } from './discover'
...
export function generateApiFile(
  ...
  const entries = functions
    .filter(fn => {
      if (!argsOnly) return true
      // The args-only registry serves scheduler codec-arg encoding ONLY —
      // functions whose args carry no codecs encode as identity and need no
      // entry. Filtering keeps this statically-imported file O(codec-args
      // functions), not O(all functions) (~0.13 MB per endpoint-file at
      // harness scale would otherwise re-create a memory cliff ≈ N=450).
      return fn.zodArgs ? schemaSubtreeHasCodec(fn.zodArgs as $ZodType) : false
    })
    .map(fn => {
...
type PathTree = Map<string, { node: PathTree | null; leaf?: ModelCodecPath }>

function buildPathTree(paths: ModelCodecPath[]): PathTree {
  const root: PathTree = new Map()
  for (const p of paths) {
    let cur = root
    for (let i = 0; i < p.path.length; i++) {
      const seg = String(p.path[i])
      const last = i === p.path.length - 1
      if (!cur.has(seg)) cur.set(seg, { node: last ? null : new Map() })
      const entry = cur.get(seg)!
      if (last) {
        entry.leaf = p
      } else {
        entry.node ??= new Map()
...
function emitCodecExpr(
  p: ModelCodecPath,
  codecRef: ((codec: $ZodType) => string | null) | undefined
): string | null {
  let expr: string | null
  if (p.isDate) {
    expr = 'zx.date()'
  } else {
    expr = codecRef?.(p.codec) ?? null
  }
  if (expr === null) return null
  if (p.nullable) expr = `z.nullable(${expr})`
  if (p.optional) expr = `z.optional(${expr})`
  return expr
}
...
function emitTree(
  ...
  for (const key of [...tree.keys()].sort()) {
    ...
    let valueExpr: string | null
    if (entry.leaf) {
      valueExpr = emitCodecExpr(entry.leaf, codecRef)
    } else if (key === '*') {
      valueExpr = null // handled by parent below; '*' children resolved here:
    } else {
      valueExpr = emitTree(entry.node!, indent + '  ', codecRef)
    }
    if (key === '*') {
      // Array element: this level IS the array; the caller wraps. Represent
      // by returning z.array(<element>) directly — an object level never
      // mixes '*' with named keys (data shapes can't).
      const inner = entry.leaf
        ? emitCodecExpr(entry.leaf, codecRef)
        : emitTree(entry.node!, indent + '  ', codecRef)
      if (inner === null) return null
      return `z.array(${inner})`
    }
    if (valueExpr === null) return null
    lines.push(`${indent}  ${key}: ${valueExpr},`)
  }
...
`
  ...
  for (const m of sorted) {
    ...
    const walked = walkModelCodecPaths(doc)

    // Custom-codec resolution: identity match against discovered standalone
    // codecs. Imports are per-file; collect them as the tree emits.
    ...
    if (walked.supported) {
      if (walked.paths.length === 0) continue // codec-free table: no descriptor
      const tree = buildPathTree(
        [...walked.paths].sort((a, b) => a.path.join('.').localeCompare(b.path.join('.')))
      )
      const schemaExpr = emitTree(tree, '', codecRef)
      if (schemaExpr !== null) {
        const needsZx = walked.paths.some(p => p.isDate)
        const importLines = [
...
`
        for (const p of walked.paths) {
          fingerprint.update(
            `${m.tableName}|${p.path.join('.')}|${p.isDate ? 'date' : 'custom'}|${p.optional ? 'o' : ''}${p.nullable ? 'n' : ''};`
          )
        }
      }
    }
    ...
    if (js === null) {
      ...
      const reason = walked.supported
        ? 'custom codec without an importable standalone reference'
        : walked.reason
