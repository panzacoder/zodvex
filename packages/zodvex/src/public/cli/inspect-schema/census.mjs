...
const isSchema = value =>
  value !== null && typeof value === 'object' && typeof value._zod?.def?.type === 'string'
...
export function schemaCensus(schema) {
  ...
  if (roots.some(root => !isSchema(root))) throw new Error('Incomplete Zod table map')
...
export function countSchemas(roots) {
  ...
  while (stack.length) {
    ...
    seen.add(schema)
    if (seen.size > 1000000) throw new Error('Diagnostic graph size budget exceeded')
    const { def, traits } = schema._zod
    const kind = kinds.has(def.type) ? def.type : 'unrecognized'
    byKind[kind] = (byKind[kind] ?? 0) + 1
    if (kind === 'unrecognized') unknown++
    implementations[
      traits?.has('ZodMiniType') ? 'mini' : traits?.has('ZodType') ? 'classic' : 'core'
    ]++
    ...
    for (const key of childKeys)
      if (descriptors[key] && 'value' in descriptors[key]) stack.push(descriptors[key].value)
    ...
    for (const value of dataValues(def)) if (typeof value === 'function') callbacks.add(value)
  ...
  return {
    ...
    implementations,
    schemasByKind: Object.fromEntries(Object.entries(byKind).sort(([a], [b]) => a.localeCompare(b)))
  }
}
