 ...
 */
/**
 * Module-scoped so the registry-miss note fires once per path per process.
 * A per-helpers set resets on every Convex invocation (ctx is wrapped per
 * call), which spammed hot paths on every request (#101).
 */
const warnedPaths = new Set<string>()

/** @internal Test-only: clears the process-wide registry-miss dedup set. */
export function __resetRegistryMissWarnings(): void {
  warnedPaths.clear()
}
  ...
  function encodeArgs(ref: FunctionReference<any, any, any, any>, args: any): any {
    ...
    if (!entry?.args) {
      if (entry === undefined && !warnedPaths.has(path)) {
        ...
        console.debug(
          `[zodvex] No registry entry for "${path}" — args/returns pass through unencoded. ` +
            'This is expected when the target is a raw (non-zodvex) function; if it should be ' +
            'codec-aware, run `zodvex generate` to update the registry.'
        )
      }
      return args
    }
