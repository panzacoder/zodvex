...
/** Ensures _zodvex/api.ts exists so user modules can import zodvexRegistry during discovery. */
function ensureStubApi(zodvexDir: string): void {
  const apiPath = path.join(zodvexDir, 'api.ts')
  if (fs.existsSync(apiPath)) return
  fs.mkdirSync(zodvexDir, { recursive: true })
  fs.writeFileSync(
    apiPath,
    '// AUTO-GENERATED by zodvex — do not edit\n// Stub created for initial codegen bootstrap\n\nexport const zodvexRegistry = {} as const\n'
  )
}
