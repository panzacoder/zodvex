...
describe('generateServerFile', () => {
  it('emits a single TS file with context types, lazy thunks, and a pre-wired initZodvex', () => {
    const { js, dts } = generateServerFile([])

    // Single TS source returned via js slot; dts unused.
    expect(dts).toBe('')

    // Header + value-level imports — the library's initZodvex aliased so
    // the export doesn't collide. (zx is no longer imported here; the
    // tableMap construction lives in api.js.)
    expect(js).toContain('// AUTO-GENERATED by zodvex')
    ...
    // Lazy registry thunk inlined (was the separate api.lazy.js file).
    expect(js).toContain("import('./api.js').then(m => m.zodvexRegistry)")

    // Lazy tableMap thunk reads from api.js (same-directory dynamic import
    // — Convex's runtime forbids cross-boundary dynamic imports).
    expect(js).toContain("import('./api.js').then(m => m.zodTableMap)")
