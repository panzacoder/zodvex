...
describe('generateServerFile', () => {
  it('emits a single TS file with context types, lazy thunks, and a pre-wired initZodvex', () => {
    ...
    // Header + value-level imports (zx for the inline tableMap, the
    // library's initZodvex aliased so the export doesn't collide).
    expect(js).toContain('// AUTO-GENERATED by zodvex')
    expect(js).toContain("import { zx } from 'zodvex'")
    ...
    // Lazy registry thunk for actions only (Node runtime supports
    // dynamic imports; Q/M V8 sandbox doesn't, see static tableMap below).
    ...
    expect(js).toContain('const _tableMap = {')
