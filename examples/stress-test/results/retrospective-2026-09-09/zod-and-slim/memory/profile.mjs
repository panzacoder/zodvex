...
export function createGraphFactory(api) {
	...
	return function buildGraph({ count, width, profile, entries = 0 }) {
		...
		const add = (name, fields) => {
			if (modeled) {
				counters.fields += Object.keys(fields).length;
				models[name] = api.defineZodModel(name, fields, api.schemaHelpers ? undefined : { schemaHelpers: false });
			} else {
		...
		const schema = modeled
			...
			: native
				...
				: null;
		// Slim models omit .schema; retain the same table-map insert alias for both modes.
		if (modeled) for (const name of Object.keys(models)) sourceSchemas[name] = schema.__zodTableMap[name].insert;
		const names = Object.keys(models);
		const registry = {};
		for (let i = 0; i < entries; i++) {
			const name = names[i % names.length];
			const doc = modeled ? schema.__zodTableMap[name].doc : sourceSchemas[name];
			const args = object({
				id: make("string"),
