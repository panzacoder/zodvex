...
Measures memory footprint at scale to find the OOM ceiling on Convex's 64 MB V8 isolate, across two seed **flavors**:
...
The `convex` baseline tells you how much headroom zodvex is spending, and how much room remains on top.
...
# Find OOM ceiling for all variants (convex baseline + 4 zodvex variants)
bun run stress-test
...
1. **Seeds** (`seeds/<flavor>/`) — hand-written models and endpoints covering small/medium/large complexity. The two flavors share the same 7 models (user, task, project, comment, document, activity, notification) with the same field shapes, so the only difference between variants is the library under test.
